...
void ReadCTP(struTouch *nowctp)
{
    ...
    {
        if ((nowctp->ctpxy.ctp_x == 0xFFF) && (nowctp->ctpxy.ctp_y == 0xFFF))
        {
            nowctp->ctpmainstatus = NO_TOUCH;
        } // 现在也没有触摸
        else
        {
            nowctp->ctpmainstatus = JUST_TOUCH;
        } // 现在刚触摸
    }
    else if ((ctplast.ctpmainstatus == JUST_TOUCH) || (ctplast.ctpmainstatus == TOUCHED)) // 上一次刚触摸 或 已经触摸到了
    {
        if ((nowctp->ctpxy.ctp_x == 0xFFF) && (nowctp->ctpxy.ctp_y == 0xFFF))
        {
            nowctp->ctpmainstatus = JUST_POP;
        } // 刚弹出
        else
        {
            nowctp->ctpmainstatus = TOUCHED;
        } // 已经按下
    }
    ...
    if (nowctp->ctpmainstatus == TOUCHED)
    {
        ...
        {
            nowctp->xmove = RIGHT_MOVING;
        }
        else if (nowctp->dx < 0)
        {
            nowctp->xmove = LEFT_MOVING;
        }
        ...
        {
            nowctp->ymove = DOWN_MOVING;
        }
        else if (nowctp->dy < 0)
        {
            nowctp->ymove = UP_MOVING;
        }
    }
    else
    {
...
void Proc_Touch_inMainWin(struTouch nowctp, uint16_t *xfreqleft, FunctionalState *disp_freq_en)
{
    if (nowctp.ctpmainstatus == TOUCHED)
    { // 触屏在主窗口内
        if ((nowctp.ctpxy.ctp_x >= FFT_MAINWIN_X0) && (nowctp.ctpxy.ctp_x < (((uint16_t)(SAMPLE_FREQ / FFT_MAINWIN_SPECT_SUM_NUMB) + FFT_MAINWIN_X0))) &&
            (nowctp.ctpxy.ctp_y > FFT_MAINWIN_Y1_2) && (nowctp.ctpxy.ctp_y < FFT_MAINWIN_Y0))
        {
            *xfreqleft = (nowctp.ctpxy.ctp_x - FFT_MAINWIN_X0) * FFT_MAINWIN_SPECT_SUM_NUMB;
            if (*xfreqleft > (FFT_LENGTH / 2 - FFT_SUBWIN_SPECTRUM_MAX))
            {
                *xfreqleft = FFT_LENGTH / 2 - FFT_SUBWIN_SPECTRUM_MAX;
            }
            *disp_freq_en = ENABLE;
        }
    }
...
void Proc_Touch_inSubWin(struTouch nowctp, uint16_t *xfreqleft, FunctionalState *disp_freq_en)
{
    ...
    if (nowctp.ctpmainstatus == TOUCHED)
    { // 触屏在子窗口内
        if ((nowctp.ctpxy.ctp_x > FFT_SUBWIN_X0_1) && (nowctp.ctpxy.ctp_x < FFT_SUBWIN_X1_1) &&
            (nowctp.ctpxy.ctp_y > FFT_SUBWIN_Y1_1) && (nowctp.ctpxy.ctp_y < FFT_SUBWIN_Y0_1))
        {
            if ((nowctp.xmove == LEFT_MOVING) || (nowctp.xmove == RIGHT_MOVING))
            {
                tmp16 = (int16_t)(*xfreqleft);
                tmp16 -= nowctp.dx;
                if (tmp16 > (FFT_LENGTH / 2 - FFT_SUBWIN_SPECTRUM_MAX / 2))
                {
                    tmp16 = FFT_LENGTH / 2 - FFT_SUBWIN_SPECTRUM_MAX / 2;
                }
                else if (tmp16 < 0)
                {
                    tmp16 = 0;
                }
                *xfreqleft = (uint16_t)tmp16;
                *disp_freq_en = ENABLE;
            }
        }
    }
